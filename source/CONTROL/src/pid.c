                 ...
                 float_t target,float_t measure)
{
    ...
    // if(target-measure>30)
    //     PID_InfoStruct->Error=30;
    // else if(target-measure<-30)
    //     PID_InfoStruct->Error=-30;
    // else
                 ...
                 float_t target,float_t measure)
{
    ...
    PID_InfoStruct->PrevPrevError=PID_InfoStruct->PrevError;
    PID_InfoStruct->PrevError=PID_InfoStruct->Error;
    if(target-measure>200)
        PID_InfoStruct->Error=200;
    else if(target-measure<-200)
        PID_InfoStruct->Error=-200;
    else
        PID_InfoStruct->Error=target-measure;
...
void PID_ParamInit(void)
{
    ...
    //横滚角速度环
    // PID_RollRateParam.P=0;//2~2.5等幅振荡，加I之后乘0.6//1.2
    // PID_RollRateParam.I=0;//0.1
    // PID_RollRateParam.D=0;//0.5
    
    PID_RollRateParam.P=2.0f;
    PID_RollRateParam.I=0.001f;//0x001f
